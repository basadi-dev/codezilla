...
use crate::system::domain::{
    ...
    ConversationItem, ItemKind, ModelSettings, ToolCall, ToolExecutionContext, ToolListingContext,
    ToolResult, TokenUsage, TurnStatus, ThreadStatus, UserInput,
};
...
impl TurnExecutor {
    pub async fn run_turn(
        ...
        let thread = self.runtime.load_thread(&params.thread_id).await?
            .ok_or_else(|| anyhow!("thread_not_found: {}", params.thread_id))?;

        {
            let mut thread = thread.lock().await;
            let turn = thread.turns.iter_mut()
            ...
            self.runtime.inner.persistence_manager.update_turn(&turn.metadata)?;
        }

        for input in params.input.clone() {
            self.append_user_input(&params.thread_id, &turn_id, &input).await?;
        }
        ...
        loop {
            ...
            let (thread_metadata, items, cancel_token) = {
                ...
                let turn = thread.turns.iter()
                    .find(|t| t.metadata.turn_id == turn_id)
                    .ok_or_else(|| anyhow!("turn_not_found: {turn_id}"))?;
                let mut items = Vec::new();
                for lt in &thread.turns { items.extend(lt.items.clone()); }
                (thread.metadata.clone(), items, turn.cancel_token.clone())
            };

            let cwd = params.cwd.clone()
                .or(thread_metadata.cwd.clone())
                .unwrap_or_else(|| self.runtime.inner.effective_config.working_directory.clone());
            let permission_profile = params.permission_profile.clone().unwrap_or_else(|| {
                self.runtime.inner.permission_manager
                    .resolve_permission_profile(&self.runtime.inner.effective_config, &cwd)
            });
            let approval_policy = params.approval_policy.clone()
            ...
            let tools = self.runtime.inner.tool_orchestrator.list_available_tools(&listing);
            let request = super::model_gateway::ModelRequest {
                model_settings: params.model_settings.clone().unwrap_or_else(|| ModelSettings {
                    model_id: thread_metadata.model_id.clone(),
                    provider_id: thread_metadata.provider_id.clone(),
                    reasoning_effort: self.runtime.inner.effective_config.model_settings.reasoning_effort.clone(),
                    summary_mode: None,
                    service_tier: None,
            ...
            let mut response = self.runtime.inner.model_gateway.start_response(request).await?;
            ...
            while let Some(event) = response.recv().await {
                ...
                match event {
                    ...
                    ModelStreamEvent::ToolCalls(calls) => { tool_calls.extend(calls); }
                    ModelStreamEvent::Completed(usage) => { final_usage = usage; }
            ...
            if let Some(item_id) = assistant_item_id {
                let item = ConversationItem {
                    item_id, thread_id: params.thread_id.clone(), turn_id: turn_id.clone(),
                    created_at: now_seconds(), kind: ItemKind::AgentMessage,
            ...
            if tool_calls.is_empty() {
                return self.complete_turn(&params.thread_id, &turn_id, final_usage).await;
            }

            for call in tool_calls {
                let call_item = ConversationItem {
                    item_id: format!("item_{}", Uuid::new_v4().simple()),
                    thread_id: params.thread_id.clone(), turn_id: turn_id.clone(),
                    created_at: now_seconds(), kind: ItemKind::ToolCall,
                    payload: serde_json::to_value(&call)?,
                };
                self.persist_turn_item(call_item).await?;

                let action = action_for_tool_call(&call, &cwd);
                if self.runtime.inner.permission_manager.requires_approval(&action, &approval_policy, &cwd) {
                    let approval = self.runtime.inner.approval_manager.create_approval(ApprovalRequest {
                    ...
                    let transcript = self.runtime.inner.persistence_manager
                        .read_thread(&params.thread_id)?.items;
                    let resolution = self.runtime.inner.approval_manager
                        .wait_for_approval(&approval.request.approval_id, 300, &transcript).await?;
                    self.runtime.publish_event(
                    ...
                    if resolution.decision != ApprovalDecision::Approved {
                        ...
                        self.persist_tool_result(&params.thread_id, &turn_id, &denial).await?;
                        continue;
                    }
                }

                let result = self.runtime.inner.tool_orchestrator.execute(
                    &call,
                    ToolExecutionContext {
                        thread_id: params.thread_id.clone(), turn_id: turn_id.clone(),
                        cwd: cwd.clone(), permission_profile: permission_profile.clone(),
                        approval_policy: approval_policy.clone(),
                    },
                ).await.unwrap_or_else(|e| ToolResult {
                    tool_call_id: call.tool_call_id.clone(), ok: false,
                    output: json!({ "error": e.to_string() }),
                    error_message: Some(e.to_string()),
                });
                self.persist_tool_result(&params.thread_id, &turn_id, &result).await?;
            }
        }
    }
    ...
    async fn append_user_input(&self, thread_id: &str, turn_id: &str, input: &UserInput) -> Result<()> {
        if let Some(text) = &input.text {
            self.persist_turn_item(ConversationItem {
                item_id: format!("item_{}", Uuid::new_v4().simple()),
                thread_id: thread_id.into(), turn_id: turn_id.into(),
                created_at: now_seconds(), kind: ItemKind::UserMessage,
        ...
        if let Some(image) = &input.image {
            self.persist_turn_item(ConversationItem {
                item_id: format!("item_{}", Uuid::new_v4().simple()),
                thread_id: thread_id.into(), turn_id: turn_id.into(),
                created_at: now_seconds(), kind: ItemKind::UserAttachment,
    ...
    pub async fn persist_turn_item(&self, item: ConversationItem) -> Result<()> {
        ...
        if let Some(thread) = self.runtime.load_thread(&item.thread_id).await? {
            let mut thread = thread.lock().await;
            if let Some(turn) = thread.turns.iter_mut().find(|t| t.metadata.turn_id == item.turn_id) {
                turn.items.push(item.clone());
            }
        }
        self.runtime.publish_event(
            crate::system::domain::RuntimeEventKind::ItemCompleted,
            Some(item.thread_id.clone()), Some(item.turn_id.clone()),
            serde_json::to_value(&item)?,
        ).await?;
        Ok(())
    }

    async fn persist_tool_result(&self, thread_id: &str, turn_id: &str, result: &ToolResult) -> Result<()> {
        let item = ConversationItem {
            item_id: format!("item_{}", Uuid::new_v4().simple()),
            thread_id: thread_id.into(), turn_id: turn_id.into(),
            created_at: now_seconds(), kind: ItemKind::ToolResult,
            payload: serde_json::to_value(result)?,
        };
        self.persist_turn_item(item).await
    }

    pub async fn complete_turn(&self, thread_id: &str, turn_id: &str, usage: TokenUsage) -> Result<()> {
        ...
        let metadata = {
            ...
            self.runtime.inner.persistence_manager.update_thread(&thread.metadata)?;
            metadata
        };
        self.runtime.publish_event(
            crate::system::domain::RuntimeEventKind::TurnCompleted,
            Some(thread_id.into()), Some(turn_id.into()),
    ...
    async fn drain_steering(&self, thread_id: &str, turn_id: &str) -> Result<()> {
        let thread = self.runtime.load_thread(thread_id).await?
            .ok_or_else(|| anyhow!("thread_not_found: {thread_id}"))?;
        let steering = { let mut t = thread.lock().await; std::mem::take(&mut t.pending_steering) };
...
fn action_for_tool_call(call: &ToolCall, cwd: &str) -> ActionDescriptor {
    let category = match call.tool_name.as_str() {
        ...
        "write_file" | "create_directory" | "remove_path" | "copy_path" => ApprovalCategory::FileChange,
        _ => ApprovalCategory::Other,
    };
    let paths = match call.tool_name.as_str() {
        "write_file" | "create_directory" | "remove_path" => call.arguments.get("path")
            .and_then(Value::as_str).map(|p| vec![p.to_string()]).unwrap_or_else(|| vec![cwd.into()]),
        "copy_path" => vec![
            call.arguments.get("source").and_then(Value::as_str).unwrap_or(cwd).to_string(),
            call.arguments.get("target").and_then(Value::as_str).unwrap_or(cwd).to_string(),
        ],
        _ => vec![cwd.into()],
    };
    ActionDescriptor {
        action_type: call.tool_name.clone(),
        command: call.arguments.get("argv").and_then(Value::as_array).map(|values| {
